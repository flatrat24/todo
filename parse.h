...
void testParseItem(char *input, todoItem *output) {
...
todoItem parseItem(char *input) {
    todoItem returnItem;
    char description[DESCRIPTIONLENGTH] = {'\0'};

    char *token = strtok(input, " ");
    if (isCompletionFormat(token) == 0) {
        parseCompletion(token, &returnItem.completion);
        token = strtok(NULL, " ");
    } else {
        returnItem.completion = 'o';
    }
    if (isDateFormat(token) == 0) {
        if (returnItem.completion == 'x') {
            parseDate(token, &returnItem.completionDate);
            token = strtok(NULL, " ");
            if (isDateFormat(token) == 0) {
                parseDate(token, &returnItem.creationDate);
                token = strtok(NULL, " ");
            }
        } else if (returnItem.completion == 'o') {
            parseDate(token, &returnItem.creationDate);
            token = strtok(NULL, " ");
        }
    }
    if (isPriorityFormat(token) == 0) {
        parsePriority(token, &returnItem.priority);
        token = strtok(NULL, " ");
    }
    while (token != NULL) {
        if (isContextFormat(token) == 0) {
            parseContext(token, returnItem.context);
        } else if (isProjectFormat(token) == 0) {
            parseProject(token, returnItem.project);
        } else if (isDueDateFormat(token) == 0) {
            parseDueDate(token, &returnItem.dueDate);
        } else if (strlen(description) + strlen(token) <= DESCRIPTIONLENGTH) {
            strcat(description, token);
            strcat(description, " ");
        }
    ...
    description[strlen(description) - 1] = '\0';
    strcpy(returnItem.description, description);
...
void formatItem(todoItem item, char *output) {
    ...
    if (item.project != "") {
        ...
        formatContext(item.project, temp);
        strcat(output, temp);
        strcat(output, " ");
    }
